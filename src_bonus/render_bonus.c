...
/*   Updated: 2024/02/18 22:34:10 by rabou-rk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_3d_game(t_game *game, int j, float i)
{
	...
	while (l < end)
	{
		if (there_is_a_wall(game, game->wall_hit_x, game->wall_hit_y) == 2)
		{
			if (game->vert == 1)
				my_mlx_pixel_put(game->img2, j, l, get_xpm_color(game,
						game->door_t, game->wall_hit_x, l
						+ (game->player->pro_wall_h) / 2 - (HEIGHT / 2)));
			if (game->vert == 0)
				my_mlx_pixel_put(game->img2, j, l, get_xpm_color(game,
						game->door_t, game->wall_hit_y, l
						+ (game->player->pro_wall_h) / 2 - (HEIGHT / 2)));
		}
		else
...
int	render(void *param)
{
	...
	mlx_string_put(game->mlx, game->win, WIDTH / 2, HEIGHT / 2, 16777215, "+");
	update(game);
	return (0);
}
